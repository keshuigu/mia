...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        # 优化label嵌入：L2归一化+两层MLP
        if label_dim:
            self.label_norm = torch.nn.functional.normalize
            self.map_label_mlp = torch.nn.Sequential(
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        ...
        # 优化label嵌入
        if self.map_label_mlp is not None and class_labels is not None:
            tmp = class_labels
            if self.training and self.label_dropout:
                tmp = tmp * (torch.rand([x.shape[0], 1], device=x.device) >= self.label_dropout).to(tmp.dtype)
            tmp = self.label_norm(tmp, dim=1)  # L2归一化
            emb = emb + self.map_label_mlp(tmp)
...
@persistence.persistent_class
class SongUNetOld(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
