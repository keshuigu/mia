...
# 屏蔽 insightface/onnxruntime provider/model 加载等冗余输出
import sys, os
import builtins
...
def filtered_print(*args, **kwargs):
    ...
    if (
        "Applied providers:" in msg
        or "model ignore:" in msg
        or "set det-size:" in msg
        or "find model:" in msg
    ):
        return
...
@click.option(
    "--data", help="Path to the dataset", metavar="ZIP|DIR", type=str, required=False
)
@click.option(
    "--cond",
...
@click.option(
    ...
    metavar="PKL|URL",
    type=str,
    # default='/home/yu/workspace/mia/training-runs/00039-celeba_feature-cond-ddpmpp-edm-gpus1-batch16-fp32-model3_run/network-snapshot-002504.pkl'
)
# default='https://nvlabs-fi-cdn.nvidia.com/edm/pretrained/edm-ffhq-64x64-uncond-vp.pkl'
@click.option(
    "--resume", help="Resume from previous training state", metavar="PT", type=str,
    default="/home/yu/workspace/mia/training-runs/00041-celeba_feature-cond-ddpmpp-edm-gpus1-batch16-fp32-model4_run/training-state-008012.pt",
)
...
def main(**kwargs):
    ...
    c = dnnlib.EasyDict()
    # 使用自定义的 CelebAFeatureDataset，img_dir为图片目录，feature_npz为特征文件
    # c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=opts.data, use_labels=opts.cond, xflip=opts.xflip, cache=opts.cache)
    c.dataset_kwargs = dnnlib.EasyDict(
        class_name="training.dataset.CelebAFeatureDataset",
    ...
    if opts.precond == "vp":
        ...
        c.loss_kwargs.class_name = "training.loss.EDMLossWithIDLossNoGradArcface"
        # c.loss_kwargs.class_name = 'training.loss.EDMFeatureCondMaskLoss'
...
"""
python train.py --outdir=training-runs  --cond=1 --arch=ddpmpp
"""
